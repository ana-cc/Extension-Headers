...
the action to be perfomed when encountering an unrecognised header.  When set to ``00", a router should
...
DST EHs. This opens TCP connections using IPv6 packets
including an EH from a crowd-sourced pool of clients and evaluates the number of
...
attempts to open a connection using a packet that includes a DST EH were successful, but close to 0\%
...
Traceroute packets were sent that include a PadN Option inserted in a DST and/or HBH EH. This Option
...
PATHSpider~\cite{learmonth2016pathspider}, a path transparency testing tool. This works by performing consecutive tests to the same target server, both with a packet that has no EH and one that includes an EH.
...
This is the proportion of paths where probe packets include an EH
...
The impact of EH size on the traversal is explored in dataset R2 - Size. Packets thatb inclyde an EH between 16B and 64B were sent from the ATLAS vantage points to 5 target destinations.  This experiment was repeated by including the
...
size of EH and split by transport. These results are compared to other destinations in Fig.~\red{fig:main-size}.}
...
Fig.~\ref{fig:sizes} only considers results where the test successfully traversed the path using a packet that includes an 8B EH 
...
In many cases, a packet that includes a HBH EH sent from a vantage point is dropped within the AS where the vantage point is located, i.e. the initial AS (68\% UDP, 74\% TCP). Similarly, a notable fraction of packets thart include a DST EH (5\% UDP, 25\% TCP) are also dropped within the
...
In some cases, the traversal for packets that include a HBH EH is significantly
...
If entropy is not extracted in a way that accounts for the EH chain, this could have two implications: it could mean that the packet that includes an EH to measure a path, or to detect a path property, will not observe the same path. This could also potentially result in packet reordering within a flow. 
...
is compared with packets that include a DST and HBH EH.  
...
measurements require the server to process the packet That include the EH and
reply.
...
The estimates of the actual support of these EHs in ASes are conservative (under-estimated). A destination AS not reachable from any vantage point may well support them, but could be masked by an upstream AS that drops packets that include an EH.
...
For tests packets that include a
...
Dec 2022.  The table shows successful traversal for a packet that include an 8B Pad N Option for both DST and
...
\subsubsection{What is the expected traversal for a packet that include an EH sent on an Internet path?}
First, we consider whether we found evidence that including an EH is expected to traverse an Internet path.
...
A smaller EH Chain currently provides more consistent Internet forwarding, and we suggest that until routers are updated, new options ought to be sized so that the packets include an EH Chain of less than 40B.
...
Traffic flows that use a mix of packets that include an EH and packets without, must anticipate that these packets might not take the same Internet path.
This motivates re-considering use of the FL for load-balancing~\cite{RFC6437}. We note that modern operating systems set the FL on packets belonging to a traffic flow~\cite{os-fl}, and some routers do already use this to perform load balancing~\cite{lb-classification}, as shown in Fig.~\ref{fig:paths-fl}. However, the FL has also been used for other purposes~\cite{flow-label-approaches} including mobility and traffic engineering~\cite{traffic-eng}. We argue, following the recommendation in~\cite{RFC6437}, that using the FL for entropy in load-balancing devices would mitigate the need to parse the entire IPv6 header chain and would also prevent packet reordering. This will enable new use-cases for EH.


\subsubsection{Can new Options be defined and used across the Internet?}
Our  data  shows that packets that include a DST can already traverse many paths both across the Internet, and at the server and network edge. 
This traversal does not depend on the type of Option (see Table~\ref{tbl:option_type_support}), which is important because it suggests that IPv6 an be extended by defining new Options that can be used on any path that allows EH processing. 
...
This paper presents an extensive measure campaign and provides novel results indicating the traversal across Internet paths of packets that include either a HBH or a DST EH. It is the first detailed study of treatment by the routers along an IPv6 path, considering both access and server edge networks. These results are used to understand whether EHs can be used to extend IPv6.
...
We also observed that the inclusion of an EH in a packet can impact the set of forwarding paths when the network layer performs load-balancing, although we assert that this can be mitigated by senders setting and ECMP routers using the FL.
