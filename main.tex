...
\hline
Test whether a consistent path is used 			& Paris Traceroute & R3- Paris        & Jan '23           & UDP               \\
\hline
...
port and, in the case of half of the vantage points, the Flow Label (FL).
Each set of 16 measurements was repeated five times.
We also repeated this test at a later date several months later using 32 Paris variations from a subset of Atlas 430 vantage points for which the IPv6 FL setting behaviour was known.
...
\begin{figure}[htbp]
    ...
    \caption{Traversal for TCP and UDP packets including HBH and DST EHs of three different sizes from the Atlas vantage points to four target servers, with UDP transport.}
    \label{fig:main-size}
\end{figure}
...
router inserted it. This implies that router had to parse the EH chain to analyse
...
that drops occur more frequently on paths where MSS Clamping is used. This problem is expected to reduce when router protocol stacks are updated to support EHs. 
...
The test was subsequently repeated with 32 Paris variations from 430 Atlas vantage points, for which the FL setting behaviour was previously measured.
...
\centering
  ...
  \caption{Role of the FL in the number of paths detected by Paris Traceroute in ~430
source-destination pairs, averaged over five measurement runs, each using 32 Paris variations (dataset R3). Setting the FL for a measurement results in between 0.5 and 1.2 extra paths detected on average by Paris Traceroute.}
  \label{fig:paths-fl}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%5
...
query over UDP to a DNS server or a TCP SYN to a web server including a PadN option
...
both including and without EHs. 
...
rate-limiting.  This rate-limiting was also observed for ICMP messages generated by other routers
...
We suggest it is possible to incrementally extend IPv6 by only utilising an EH when a path is found to support it,
...
This paper presents novel results on the traversal of packets that include an HBH or DST EH across Internet paths, seeking to determine whether these EHs can be used to extend IPv6 capabilities. It is the first detailed study of treatment by the routers along an IPv6 path, within access and server edge networks.

The successful reception across a current IPv6 path can currently depend on the type of included EH, its size, and on the transport protocol utilized. End-to-end transmission is strongly influenced by the type of network of the source an destination.
