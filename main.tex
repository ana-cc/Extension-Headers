...
IPv6 Extension Headers (EHs)~\cite{RFC8200} are optional headers 
added to the basic IPv6 header that provide extra functionality and features in
IPv6 networks.  The use of extension headers allows for a flexible and
extensible design of IPv6 that can support a variety of new networking
features and technologies.
 
Despite the potential benefits of IPv6 EHs, studies have shown that their use
has not been widespread~\cite{rfc9098}. A reason often cited for this is that some
...
Additionally, some network administrators may be hesitant to enable extension
headers due to security concerns, as they can potentially be used to bypass
security mechanisms or launch attacks~\cite{naagas2021deh}.

A plausible process that has resulted in the limited usage of IPv6 extension
headers (EHs) has recently been described in an IETF
document~\cite{ietf-v6ops-hbh-03}. Authors claims  that early routers
predominantly processed EHs in software, as ASIC implementations of processing
of IPv6 extensions were scarce. Consequently, the processing of EHs occurred
primarily in the control plane of routers, where longer processing times
(slow-path) were needed for hop-by-hop extensions. Ultimately, this led to an
increase of per-packet processing time and a decrease in throughput.  To
overcome this challenge, routers began disabling the processing of extension
headers, or even dropping packets with EHs altogether.  In turn, this
discouraged the development of new extension headers and further integration
...
that there are still prevalent pathologies in certain Internet areas that could
hinder the widespread adoption of IPv6 EHs if not addressed.
...
studies are here attributed to either too narrow datasets or an excessive focus
...
the path traversal rate of packet carrying Destination Options can be as
high as 90\% across a diverse set of Internet paths. However, the path
traversability of Hop-by-Hop EHs remains restricted. On the other hand, when we
characterise Autonomous Systems (AS) traversability, if we exclude sporadic
cases with IP-in-IP tunnels  or specific policies in acceess networks, the vast
majority of ASes allow transparent forwarding of EHs.

This paper also presents novel insights on how the use of EHs in packet compositions
impacts the routing process from source to destination. Our research findings
demonstrate that the inclusion of EHs in packets of the same flow results in a
broader distribution of routing paths. Although one could interpret this  as an
alternative approach to extracting entropy from EHs, it is more likely a
pathological outcome of front-end load balancing systems not properly equipped
to handle packets carrying EHs.

We claim that eliminating these pathologies would open up opportunities for
the standards community to propose a range of innovative ideas based on EHs.
Such proposals include sending larger packets~\cite{rfc9268} and monitoring network
performance~\cite{rfc8250}~\cite{ietf-ippm-ioam-ipv6-options-10}.

The rest of the paper is so organised.  Section~\ref{sec:background} presents
the required background on IPv6 EHs and the historical challenges related to
its deployment.  The previous literature on Internet wide measurement of IPv6
